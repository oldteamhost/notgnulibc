...
struct arp_ethip
{
  u8 ar_sha[ETH_ADDR_LEN]; /* sender hardware address */
  u8 ar_spa[NS_INET_ADDRSZ];  /* sender protocol address */
  u8 ar_tha[ETH_ADDR_LEN]; /* target hardware address */
  u8 ar_tpa[NS_INET_ADDRSZ];  /* target protocol address */
};
...
#define arp_pack_hdr_ethip(hdr, op, sha, spa, tha, tpa) do {\
  struct arp_header *pack_arp_p = (struct arp_header *)(hdr);\
  ...
  pack_arp_p->ar_pln = NS_INET_ADDRSZ;\
