...
void* block_from_unused_heap(size_t size)
{
  if (NULL == thread_unused_heap_start || (thread_heap_end - thread_unused_heap_start) < (size + sizeof(block_info))) {
    if (NULL == heap_used_memory_end || (_sbrk(0) - heap_used_memory_end) < (size + sizeof(block_info))) {
      if (NULL == heap_used_memory_end) {
        heap_used_memory_end = _sbrk(0);
    ...
    if((_sbrk(__getpagesize() * 100) == (void *) -1)) {
      return NULL;
    }

    thread_unused_heap_start = heap_used_memory_end;
    thread_heap_end = heap_used_memory_end + (__getpagesize());
...
{
  int num_pages = ((size + sizeof(block_info) - 1)/ __getpagesize()) + 1;
  int required_page_size = __getpagesize() * num_pages;
